...
export const findNode = (_name: string | null, _items: Node[]) => _items.findIndex((_node: Node) => _node.data.label === _name) 
...
export const nodesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(runningNode.pending, 
      (state, action: PayloadAction<void>) => {        
        // console.log("RunNode.pending");
      })
      .addCase(runningNode.fulfilled,
        (state, action: PayloadAction<void>) => {
          state.runningNode = null;
          // console.log("RunNode.fulfilled");
        }
      )
      .addCase(runningNode.rejected,
        (state) => {
          state.runningNode = null;          
...
export const runningNode = createAsyncThunk(
  ...
  async (node: Node, thunkAPI) => {
    await new Promise<void>((resolve, reject) => {
      ...
      const eventSource = new EventSource(`http://127.0.0.1:5000/nodes/run/${node.data.label}`);
